...
async def validate_credentials(request: CredentialValidationRequest):
    ...
    try:
        ...
        validation_result = credential_validator.validate_credentials(
            request.service, 
            request.credentials
        )

        # If validation fails, raise an HTTP exception
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Credential validation error: {str(e)}"
        )
...
async def decrypt_credentials(encrypted_credentials: Dict[str, Any]):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Credential decryption error: {str(e)}"
        )
